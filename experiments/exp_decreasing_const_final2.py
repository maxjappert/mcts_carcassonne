...
for type in tree_policies.keys():
    for const in consts:
        ...
        key1 = f'{type.replace("-", "")}_{const}_1'
        key2 = f'{type.replace("-", "")}_{const}_2'

        key1 = key1.replace('.', 'dot')
        key2 = key2.replace('.', 'dot')

        value1 = ['--p1',  f'decaying-{type}', '--p2', f'{type}', '--p1trainingiterations', '1000', '--p2trainingiterations', '1000', '--p1explorationterm', f'{const}', '--p2explorationterm', f'{tree_policies[type]}']
        value2 = ['--p1',  f'{type}', '--p2', f'decaying-{type}', '--p1trainingiterations', '1000', '--p2trainingiterations', '1000', '--p1explorationterm', f'{tree_policies[type]}', '--p2explorationterm', f'{const}']
