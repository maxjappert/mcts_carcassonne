...
options = [2**i for i in range(3, 7)]

for tree_policy in tree_policies.keys():
    for num_ensemble_iterations in options:
        key1 = f'{tree_policy}-{num_ensemble_iterations}-1'
        key2 = f'{tree_policy}-{num_ensemble_iterations}-2'

        value1 = ['--p1',  f'{tree_policy}', '--p2',  f'{tree_policy}', '--p1trainingiterations', f'{500}', '--p2trainingiterations', f'{1000}', '--p1explorationterm', f'{tree_policies[tree_policy]}', '--p2explorationterm', f'{tree_policies[tree_policy]}', '--p1ensembleiterations', f'{num_ensemble_iterations}']
        value2 = ['--p1',  f'{tree_policy}', '--p2',  f'{tree_policy}', '--p1trainingiterations', f'{1000}', '--p2trainingiterations', f'{500}', '--p1explorationterm', f'{tree_policies[tree_policy]}', '--p2explorationterm', f'{tree_policies[tree_policy]}', '--p2ensembleiterations', f'{num_ensemble_iterations}']
...
for algo_name, algo_cmd in ALGORITHMS.items():
    for seed in range(5):
        ...
        run.add_command(
            f"{algo_name}",
            ["java", "-Xms3500m", "-Xmx3500m",  "-jar", "{solver}", algo_cmd, "--deckseed", seed, "-v", "false"],
